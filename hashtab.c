/*
 * hashtab.c: Handling of a hashtable with uemacs-specific properties.
 *
 ...
 */
hashtab_T *hash_create(void)
{
    hashtab_T *ht;

    ht = (hashtab_T *)malloc(sizeof(hashtab_T));
    if (ht != NULL)
	hash_init(ht);
    return ht;
}
 ...
 * "hi" is invalid after this!
 */
void hash_set(hashitem_T *hi, char_u *key)
{
    hi->hi_key = key;
}
#endif
    ...
    int		minitems)		/* minimal number of items */
{
    ...
    {
	/* Allocate an array. */
	newarray = (hashitem_T *)malloc((unsigned)
